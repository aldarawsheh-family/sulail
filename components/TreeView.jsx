...
// ========== مكون الجوال: ختم ذهبي ==========
function GlowingSeal({ person, title, isActive, onClick }) {
  if (!person) {
    return (
      <div className="flex flex-col items-center relative my-1 font-cairo select-none opacity-30">
        <span className="text-[9px] text-slate-500/60 mb-1 tracking-widest bg-slate-900/50 px-2 py-0.5 rounded-full border border-slate-800/50">{title}</span>
        <div className="w-16 h-16 rounded-full border-2 border-dashed border-slate-700/50 bg-slate-900/50 flex items-center justify-center text-slate-600 text-xl">—</div>
      </div>
    );
  }
  return (
    <div className="flex flex-col items-center relative my-1 font-cairo select-none">
      {isActive && <div className="absolute w-24 h-24 bg-amber-500/15 rounded-full blur-2xl animate-pulse"></div>}
      <div onClick={() => onClick(person)}
        className={`w-[72px] h-[72px] rounded-full border-2 flex flex-col items-center justify-center p-1 text-center cursor-pointer transition-all active:scale-95 shadow-xl z-10 ${
          isActive ? 'border-amber-500 bg-gradient-to-b from-amber-950/40 to-slate-900 shadow-[0_0_20px_rgba(217,119,6,0.5)] text-amber-300' : 'border-slate-600 bg-slate-900/80 text-slate-200 hover:border-amber-500/50'
        }`}>
        <span className="text-[10px] mb-0.5">{person.status?.includes('شهيد') ? '⚔️' : person.status?.includes('انتقل') ? '🕊️' : '🍁'}</span>
        <span className="text-[11px] font-bold leading-tight truncate w-full px-0.5">{person.display_name || person.first_name || ''}</span>
      </div>
    </div>
  );
}

// ========== مكون الشجرة الموسعة للجوال ==========
function ExpandedTree({ person, persons, level, maxLevel, onNodeClick }) {
  ...
  return (
    <div className="flex flex-col items-center">
      {level > 0 && <div className="w-0.5 h-3 bg-gradient-to-b from-amber-500/30 to-amber-500/5"></div>}
      <div className="flex flex-wrap justify-center gap-1">
        {children.map(child => (
          <div key={child.id} className="flex flex-col items-center">
            ...
            >
              <span className="text-[9px]">{child.status?.includes('شهيد') ? '⚔️' : child.status?.includes('انتقل') ? '🕊️' : '🍁'}</span>
              <span className="text-[8px] font-bold leading-tight truncate w-full px-0.5 text-slate-200">{child.display_name || child.full_name || ''}</span>
            </div>
...
export default function TreeView({ persons, isAdmin, onEdit, onDelete, onAdd, branch, onUpdateBranch }) {
  ...
  useEffect(() => {
    if (rootPerson && !mobileRoot) setMobileRoot(rootPerson);
  }, [rootPerson, mobileRoot]);
                  ...
                  {level.map(person => {
                    ...
                    return (
                      <div key={person.id} className="flex flex-col items-center">
                        <GlowingSeal person={person} title="" isActive={isRoot} onClick={setSel} />
                        {showExpandBtn && (
