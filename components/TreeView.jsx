...
const [editingRootLabel, setEditingRootLabel] = useState('');
  useEffect(() => {
    if (!branch?.id) return;
    fetch(`/api/branch-roots?branchId=${branch.id}`)
      .then(r => r.json())
      .then(d => setSubRoots(d || []));
  }, [branch?.id]);
  ...
  function drawLines(ctx, node) {
    ...
    for (const ch of node.children) {
      ...
      ctx.strokeStyle = lg ? '#FFD700' : '#D4AF3720'; ctx.lineWidth = lg ? 8 : 6;
      ctx.shadowColor = lg ? '#FFD700' : '#D4AF37'; ctx.shadowBlur = lg ? 30 : 18;
      ctx.stroke(); ctx.shadowBlur = 0;
      ctx.beginPath(); ctx.moveTo(sx, sy);
      if (d) ctx.lineTo(ex, ey); else ctx.bezierCurveTo(sx, my, ex, my, ex, ey);
      ctx.strokeStyle = lg ? '#FFD700' : '#C9A84C'; ctx.lineWidth = lg ? 3 : 2;
      ctx.stroke();
    }
  ...
  function drawCards(ctx, node) {
    ...
    else { grad.addColorStop(0, '#FFF8E1'); grad.addColorStop(1, '#F0DFB0'); }
    ctx.fillStyle = grad; ctx.fill();
    ctx.strokeStyle = g ? '#FFD700' : '#D4AF37'; ctx.lineWidth = g ? 3 : 2;
    ctx.shadowColor = g ? '#FFD700' : 'transparent'; ctx.shadowBlur = g ? 20 : 0;
    ctx.stroke(); ctx.shadowBlur = 0;
    ctx.fillStyle = g ? '#1A0A00' : '#2A1A05';
  ...
  function search(v) {
    setQ(v); if (v.length < 1) { setQr([]); return; }
    const s = v.toLowerCase().trim();
    let results = persons.filter(p => getFullThreeNames(persons, p).toLowerCase().includes(s));
    results.sort((a, b) => getFullThreeNames(persons, a).length - getFullThreeNames(persons, b).length);
    setQr(results.slice(0, 10));
  }
  function findLayoutNode(node, id) {
    if (!node) return null; if (node.id === id) return node;
    if (node.children) for (const ch of node.children) { const f = findLayoutNode(ch, id); if (f) return f; }
    return null;
  }
 ...
 function go(p) {
    setQ('');
    setQr([]);
    if (!lay) { setSel(p); return; }
    if (glowActive) return;
    setGlowActive(true);
    setGlow([]);
    const anc = getAncestors(persons, p);
    const tn = findLayoutNode(lay, p.id);
    if (tn && cr.current) {
      const ct = cr.current;
      const ts = 1.2;
      const nx = tn.x * scale + pan.x;
      const ny = tn.y * scale + pan.y;
      const mx = ct.clientWidth / 2;
      const my = ct.clientHeight * 0.4;
  ...
  function handleCanvasClick(e) {
    ...
    const mx = (e.clientX - rect.left - pan.x) / scale, my = (e.clientY - rect.top - pan.y) / scale;
  ...
  function md(e) { if (e.target.closest('button') || e.target.tagName === 'CANVAS') { setDrag(true); ds.current = { x: e.clientX - pan.x, y: e.clientY - pan.y }; } }
  function mm(e) { if (!drag) return; setPan({ x: e.clientX - ds.current.x, y: e.clientY - ds.current.y }); }
  function mu() { setDrag(false); }
  function ts(e) { if (e.touches.length === 1) { setDrag(true); ds.current = { x: e.touches[0].clientX - pan.x, y: e.touches[0].clientY - pan.y }; } }
  function tm(e) { if (!drag) return; setPan({ x: e.touches[0].clientX - ds.current.x, y: e.touches[0].clientY - ds.current.y }); }
  function te() { setDrag(false); }
  function wh(e) { e.preventDefault(); setScale(s => Math.max(0.3, Math.min(2.5, s + (e.deltaY < 0 ? 0.1 : -0.1)))); }
...
async function saveSubRootStory(rootId, story, label) {
    ...
    setSubRoots(prev => prev.map(r => r.id === rootId ? { ...r, story, label } : r));
    setEditingRootId(null);
    setEditingRootLabel('');
  }

  function searchRoots(v) {
    setRootSearchQ(v); if (v.length < 1) { setRootSearchResults([]); return; }
    setRootSearchResults(persons.filter(p => getFullThreeNames(persons, p).toLowerCase().includes(v.toLowerCase().trim())).slice(0, 10));
  }

  function getPersonById(id) { return persons.find(p => p.id === id); }
    return (
    <div ref={cr} className="relative"
      style={{ minHeight: '100vh', height: '100vh', background: 'linear-gradient(180deg, #0C1828 0%, #12243A 40%, #0C1828 100%)', position: 'fixed', inset: 0, zIndex: 0, overflow: 'hidden', cursor: drag ? 'grabbing' : 'grab' }}
      onMouseDown={md} onMouseMove={mm} onMouseUp={mu} onMouseLeave={mu} onTouchStart={ts} onTouchMove={tm} onTouchEnd={te} onWheel={wh}
    >
      ...
      <div className="absolute top-24 left-1/2 -translate-x-1/2 z-20 flex items-center gap-3 w-80">
        <button onClick={() => setShowStats(true)} className="bg-[#12243A]/90 border border-[#D4AF3740] text-[#D4AF37] px-4 py-2 rounded-xl text-xs font-bold hover:bg-[#D4AF3715] transition shadow-lg whitespace-nowrap">📊</button>
      ...
      <div className="absolute bottom-6 left-6 flex flex-col gap-2 z-20">
        <button onClick={() => setScale(s => Math.min(2.5, s + 0.15))} className="w-10 h-10 bg-[#12243A]/90 rounded-xl shadow-lg text-[#D4AF37] font-bold text-lg border border-[#D4AF3740]">+</button>
        <button onClick={() => setScale(s => Math.max(0.3, s - 0.15))} className="w-10 h-10 bg-[#12243A]/90 rounded-xl shadow-lg text-[#D4AF37] font-bold text-lg border border-[#D4AF3740]">−</button>
        <button onClick={() => setScale(1)} className="w-10 h-10 bg-[#12243A]/90 rounded-xl shadow-lg text-[#D4AF37] text-xs font-bold border border-[#D4AF3740]">↺</button>
      </div>
      <canvas ref={cv} onClick={handleCanvasClick}
        style={{ position: 'absolute', top: 0, left: 0, transform: `translate(${pan.x}px, ${pan.y}px) scale(${scale})`, transformOrigin: 'top left', transition: drag ? 'none' : 'transform 0.3s ease' }} />
      ...
      {showStats && (
        <div className="fixed inset-0 z-40 flex items-center justify-center p-4" onClick={() => { setShowStats(false); setShowAddRoot(false); }}>
          ...
          <div className="relative bg-[#12243A] rounded-3xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto animate-slideUp border border-[#D4AF3740]" onClick={e => e.stopPropagation()}>
            <div className="bg-gradient-to-r from-[#2B5F8E] to-[#4A90D9] px-6 py-4 text-center">
              <button onClick={() => setShowStats(false)} className="absolute top-4 right-4 w-8 h-8 bg-white/10 rounded-full flex items-center justify-center text-white text-sm hover:bg-white/20 transition">✕</button>
              <h3 className="text-white font-bold text-lg">📊 إحصائيات الفرع</h3>
            </div>
            <div className="p-6 space-y-5">
              {rootPerson && (
                <div className="bg-[#0A1628]/40 rounded-2xl p-4 text-center">
                  ...
                  {isAdmin && (
                    <div className="mt-3">
                      <textarea value={branchStory} onChange={e => setBranchStory(e.target.value)} placeholder="نبذة عن الفرع الرئيسي..." rows={3}
                        className="w-full px-4 py-2 bg-[#0A1628]/60 border border-[#D4AF3720] rounded-xl text-right text-xs text-white placeholder:text-[#8A95A4] resize-none" />
                  ...
                  {!isAdmin && branchStory && <p className="text-white text-xs mt-3 whitespace-pre-wrap">{branchStory}</p>}
                </div>
              )}
              ...
              <div>
                ...
                {subRoots.map(sr => {
                  ...
                  if (!p) return null;
                  return (
                    <div key={sr.id} className="bg-[#0A1628]/40 rounded-2xl p-4 mb-3 text-center">
                <p className="text-white font-bold text-sm">{sr.label || p.display_name || p.full_name}</p>
                      ...
                      {isAdmin && (
                        <div className="mt-2">
                          {editingRootId === sr.id ? (
  <div>
    <input type="text" value={editingRootLabel} onChange={e => setEditingRootLabel(e.target.value)} placeholder="اسم الفرع..." 
      className="w-full px-4 py-2 bg-[#0A1628]/60 border border-[#D4AF3720] rounded-xl text-right text-xs text-white mb-2" />
    <textarea value={editingRootStory} onChange={e => setEditingRootStory(e.target.value)} rows={2}
      className="w-full px-4 py-2 bg-[#0A1628]/60 border border-[#D4AF3720] rounded-xl text-right text-xs text-white resize-none" />
    <div className="flex gap-2 justify-center mt-2">
      ...
      <button onClick={() => setEditingRootId(null)} className="bg-[#FFFFFF10] text-white px-3 py-1 rounded-lg text-xs">إلغاء</button>
    </div>
  </div>
) : (
  <div>
    ...
    <button onClick={() => { setEditingRootId(sr.id); setEditingRootStory(sr.story || ''); setEditingRootLabel(sr.label || ''); }} className="text-[#D4AF37] text-xs">✏️ تعديل</button>
                      ...
                      {!isAdmin && sr.story && <p className="text-white text-xs mt-2 whitespace-pre-wrap">{sr.story}</p>}
                    </div>
                  );
                })}
                ...
                {isAdmin && (
                  <div className="text-center mt-3">
                    {!showAddRoot ? (
                      ...
                      <div className="bg-[#0A1628]/60 rounded-2xl p-4">
                        <input type="text" placeholder="🔍 ابحث عن الشخص..." value={rootSearchQ} onChange={e => searchRoots(e.target.value)}
                          className="w-full px-4 py-2 bg-[#12243A] border border-[#D4AF3740] rounded-xl text-right text-xs text-white mb-2" />
                        {rootSearchResults.map(p => (
                          <button key={p.id} onClick={() => addSubRoot(p.id)} className="w-full text-right px-4 py-2 text-xs text-white hover:bg-[#D4AF3715] rounded-lg">
                            {getFullThreeNames(persons, p)}
                          </button>
                        ))}
        ...
        <div className="fixed inset-0 z-30 flex items-center justify-center p-4" onClick={() => setSel(null)}>
          ...
          <div className="relative bg-[#12243A] rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-slideUp border border-[#D4AF3740] max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="bg-gradient-to-r from-[#D4AF37] to-[#B49450] px-6 py-5 text-center">
              <button onClick={() => setSel(null)} className="absolute top-4 right-4 w-8 h-8 bg-black/20 rounded-full flex items-center justify-center text-[#0A1628] text-sm hover:bg-black/30 transition">✕</button>
              <div className="w-16 h-16 mx-auto mb-3 rounded-2xl bg-white/20 flex items-center justify-center text-3xl shadow-lg">{icon(sel)}</div>
            ...
            <div className="p-5 text-center space-y-4">
              ...
              <div className="space-y-3 text-xs bg-[#0A1628]/30 rounded-2xl p-4">
              ...
              <div className="bg-[#0A1628]/50 rounded-2xl p-4 border border-[#D4AF3715]"><p className="text-[10px] text-[#B49450] leading-relaxed font-heading text-center">📜 {getLineage(persons, sel)}</p></div>
              <div className="flex justify-center mt-2">
                <div style={{ width: 85, height: 85, borderRadius: '50%', border: '2.5px solid #4A90D950', background: 'linear-gradient(135deg, #4A90D910, #4A90D905)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontSize: 7, fontWeight: 900, color: '#4A90D9', lineHeight: 1.5, transform: 'rotate(-5deg)', boxShadow: '0 0 20px rgba(74,144,217,0.15)', letterSpacing: 0.5 }}>
