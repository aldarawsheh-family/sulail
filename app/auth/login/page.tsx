...
export default function LoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    if (data.error) {
      ...
      localStorage.setItem("sulail_user_role", data.user?.role_id || "");
      localStorage.setItem("sulail_branch_id", data.user?.branch_id || "");
      ...
      <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl border border-[#B49450]/20 p-8">
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          <div>
            ...
            <input type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="فلان بن فلان" className="w-full px-4 py-3.5 bg-[#F5F0E8] rounded-2xl text-right outline-none border-2 border-transparent focus:border-[#B49450] transition" required />
          </div>
          <div>
            <label className="block text-sm font-bold text-[#0A1628] mb-1">كلمة المرور</label>
            <div className="relative">
              <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="w-full px-4 py-3.5 bg-[#F5F0E8] rounded-2xl text-right outline-none border-2 border-transparent focus:border-[#B49450] transition pr-12" required />
          ...
          <button type="submit" disabled={loading} className="w-full bg-[#B49450] text-white py-3.5 rounded-2xl font-bold hover:bg-[#D4AF37] transition disabled:opacity-50">{loading ? "⏳ جاري الدخول..." : "دخول"}</button>
        </form>
        <p className="text-center text-[#8A95A4] text-xs mt-6">
          ليس لديك حساب؟ <a href="/auth/register" className="text-[#B49450] hover:underline">إنشاء حساب</a>
        </p>
      </div>
    </main>
  );
