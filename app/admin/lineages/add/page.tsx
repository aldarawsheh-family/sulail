...
export default function AddLineagePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/admin/lineages", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: name.trim(), slug, clan_id: clanId, description }),
      });

      const data = await res.json();
      if (res.ok) {
        setMessage("✅ تمت إضافة الفخذ بنجاح!");
        setName("");
        setDescription("");
  ...
  return (
    <main className="min-h-screen bg-[#FDFBF7] font-body">
      <div className="max-w-lg mx-auto px-5 py-12">
        <h1 className="text-2xl font-heading font-bold text-[#0A1628] mb-6">🌱 إضافة فخذ جديد</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <label className="block text-sm font-bold text-[#0A1628] mb-1">اسم الفخذ</label>
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="مثال: الجبل" className="w-full px-4 py-3 bg-[#F5F0E8] border border-[#B49450]/20 rounded-xl text-right outline-none focus:border-[#B49450] transition" required />
          </div>
          ...
          <button type="submit" disabled={loading} className="w-full bg-[#0A1628] text-white py-3.5 rounded-xl font-bold hover:bg-[#B49450] transition disabled:opacity-50">
            {loading ? "⏳ جاري الإضافة..." : "➕ إضافة الفخذ"}
          </button>
        </form>
      </div>
    </main>
