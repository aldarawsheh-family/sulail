...
export async function POST(request) {
  ...
  if (matchedUsers.length > 1) {
    return new Response(
      JSON.stringify({ ambiguous: true, message: "يوجد أكثر من شخص بهذا الاسم. الرجاء إدخال اسم الجد للتمييز.", count: matchedUsers.length }),
      { status: 409 }
    );
  }
  ...
  const response = new Response(
    JSON.stringify({
      success: true,
      user: { id: user.id, full_name: user.full_name, email: user.email, role_id: user.role_id, branch_id: user.branch_id },
    }),
    {
      status: 200,
      headers: {
        "Set-Cookie": [
          `sulail_token=${token}; Path=/; SameSite=Lax; Secure; HttpOnly`,
          `sulail_user_name=${encodeURIComponent(userName)}; Path=/; SameSite=Lax; Secure`,
        ].join(", "),
      },
    }
  );
